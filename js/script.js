...
//随机文字烟花内容
const randomWords = ["新年快乐", "心想事成", "万事如意", "天天开心"];
let customWord = ""; // 用户自定义文字
const wordDotsMap = {};
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 3, "Gabriola,华文琥珀,Microsoft YaHei,SimHei,Arial,sans-serif", "90px");
});
...
const helpContent = {
	...
	customWord: {
		header: "自定义文字",
		body: "输入你想要显示的文字（1-4个汉字），在文字烟花开启时，会优先显示你输入的文字。留空则使用预设的祝福语。",
	},
	autoLaunch: {
		header: "自动放烟花",
	...
	trailLength: {
		header: "拖尾长度",
		body: "调整烟花拖尾的长度。短拖尾会让动画更流畅，长拖尾会增加视觉残留效果。",
	},
};
...
const nodeKeyToHelpKey = {
	...
	customWordLabel: "customWord",
	autoLaunchLabel: "autoLaunch",
	finaleModeLabel: "finaleMode",
	hideControlsLabel: "hideControls",
	fullscreenLabel: "fullscreen",
	longExposureLabel: "longExposure",
	trailLengthLabel: "trailLength",
};
...
const appNodes = {
	...
	customWord: ".custom-word", // 自定义文字输入
	customWordLabel: ".custom-word-label",
	...
	trailLengthLabel: ".trail-length-label",
...
function renderApp(state) {
	...
	appNodes.trailLength.value = state.config.trailLength;
...
function getConfigFromDOM() {
	return {
		...
		trailLength: appNodes.trailLength.value,
	};
}
...
appNodes.trailLength.addEventListener("input", updateConfigNoEvent);
...
// 自定义文字输入监听
appNodes.customWord.addEventListener("input", (e) => {
	customWord = e.target.value.trim();
});
...
const heartShell = (size = 1) => {
	const color = Math.random() < 0.3 ? COLOR.Pink : (Math.random() < 0.5 ? COLOR.Red : randomColor());
	return {
		shellSize: size,
...
const spiralShell = (size = 1) => {
	...
	const color2 = randomColor({ notColor: color1 });
	return {
		shellSize: size,
...
function init() {
	...
	setOptionsForSelect(appNodes.trailLength, [
		{ label: "短", value: "short" },
		{ label: "正常", value: "normal" },
		{ label: "长", value: "long" },
		{ label: "超长", value: "xlong" },
	]);
...
function render(speed) {
	...
	if (store.state.config.longExposure) {
		...
		const trailLengthMap = {
			short: 0.3,
			normal: 0.175,
			long: 0.08,
			xlong: 0.02,
		};
		trailAlpha = trailLengthMap[store.state.config.trailLength] || 0.175;
	}
	trailsCtx.fillStyle = `rgba(0, 0, 0, ${trailAlpha * speed})`;
...
function getWordDots(word) {
	...
	var res = MyMath.literalLattice(word, 3, "Gabriola,华文琥珀,Microsoft YaHei,SimHei,Arial,sans-serif", fontSize + "px");
	...
	burst(x, y) {
		...
		if (typeof this.color === "string") {
			...
			// 心形烟花
				...
				const heartStartAngle = Math.random() * Math.PI;
				...
				for (let i = 0; i < this.starCount; i++) {
					...
					const heartAngle = Math.atan2(hy, hx) + heartStartAngle;
					const heartSpeed = Math.sqrt(hx * hx + hy * hy) / 16 * speed * heartScale;
			...
			// 螺旋烟花
				...
				const spiralStartAngle = Math.random() * Math.PI;
				const spiralTurns = 3; // 螺旋圈数

				for (let i = 0; i < this.starCount; i++) {
					const progress = i / this.starCount;
					const angle = progress * spiralTurns * PI_2 + spiralStartAngle;
					const spiralSpeed = progress * speed; // 速度随距离增加

					// 使用数组中的两种颜色
					const spiralColor = progress < 0.5 ? this.color[0] : this.color[1];
			...
			// 彩虹烟花
				...
				const allColors = COLOR_CODES;
				for (let i = 0; i < this.starCount; i++) {
					const progress = i / this.starCount;
					const colorIndex = Math.floor(progress * allColors.length) % allColors.length;
					const rainbowColor = allColors[colorIndex];

					const star = Star.add(
						x,
						y,
						rainbowColor,
						(Math.random() - 0.5) * PI_2,
						Math.random() * speed,
						this.starLife + Math.random() * this.starLife * this.starLifeVariation
					);
					...
					// 彩虹渐变效果 - 粒子颜色会随时间变化
					star.transitionTime = this.starLife * 0.3;
					star.secondColor = allColors[(colorIndex + 1) % allColors.length];
				}
			}
			// Normal burst
			else {
...
const soundManager = {
	...
	playSound(type, scale = 1) {
		...
		if (!canPlaySoundSelector() || simSpeed < 0.95) {
			console.log(`🔇 声音被阻止: type=${type}, soundEnabled=${soundEnabledSelector()}, simSpeed=${simSpeed}`);
			return;
		}
		...
		console.log(`🔊 播放声音: ${type}, 音量: ${scaledVolume.toFixed(2)}`);
	},
};
